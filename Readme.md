...
- This repo bootstraps that storage under **`terraform/bootstrap/`** (applied **once**, with **local** `terraform.tfstate` only for that bootstrap folder).
- The **main** stack in **`terraform/environments/main/`** uses the **`azurerm` backend** so its state is stored in a blob (e.g. key `cst8918.tfstate` in container **`state-storage`** in account **`stateblob`**, RG **`finaldevops-rg`** — see `backend.hcl.example`).

After bootstrap, **everyone** runs Terraform against that **same** backend so you all share one state and avoid duplicating infrastructure.
...
2. **Backend configuration:** copy the example and adjust if your team’s values differ:

   ```bash
   cp backend.hcl.example backend.hcl
   ```

   `backend.hcl` is **gitignored**; do not commit secrets. With **`az login`**, Terraform can use Azure AD to access the storage account (your account needs permission on that resource group or storage — **Contributor** on the subscription usually covers it). For **GitHub Actions**, use OIDC and set `use_oidc = true` in `backend.hcl` (see your CI workflow).

3. **Initialize** (downloads providers and connects the backend; may prompt to migrate state if you switched backends):
